...
// Keep generated preview files outside OneDrive and separate from publishing builds.
const previewDir=fs.mkdtempSync(path.join(os.tmpdir(),'portfolio-preview-'));
build({copyAssets:false,basePath:'',outputDir:previewDir});
...
http.createServer((req,res)=>{
 try{
  ...
  const url=new URL(req.url,'http://localhost');let file=path.resolve(previewDir,'.'+decodeURIComponent(url.pathname));
  if(!file.startsWith(previewDir+path.sep)&&file!==previewDir){res.writeHead(403);res.end();return;}
  if(!path.extname(file)){build({copyAssets:false,basePath:'',outputDir:previewDir});if(!url.pathname.endsWith('/')){res.writeHead(302,{Location:url.pathname+'/'+url.search});res.end();return;}file=path.join(file,'index.html');}
  ...
  if(!fs.existsSync(file)){res.writeHead(404,{'Content-Type':'text/html'});res.end(fs.readFileSync(path.join(previewDir,'404.html')));return;}
