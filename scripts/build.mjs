...
export function build({copyAssets=true,basePath=process.env.BASE_PATH||'',outputDir=path.join(root,'dist')}={}) {
 ...
 for(const f of ['favicon.svg','style.css','app.js','home-ripples.js','youtube-player.js','hls-player.js'])fs.copyFileSync(path.join(root,'src',f),path.join(out,f));
 return projects;
}
