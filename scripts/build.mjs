...
export function build() {
 ...
 const projects = JSON.parse(read('Assets/projects.json')).map(p=>{
  ...
  const media=fs.readdirSync(folder).filter(f=>/\.(jpe?g|png|webp|gif|avif|svg|mp4|webm|ogv|mov)$/i.test(f)).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
